...
from tf_keras.callbacks import ModelCheckpoint, CSVLogger, LearningRateScheduler

from tf_keras.layers import (Input, Embedding, Dense, BatchNormalization,
...
def evaluate_C(inputs, preprocessor, model):
    ...
    for x in evaluate_sequence:
        # Direkter TF-Aufruf umgeht die Keras-Kardinalitätsprüfung,
        # die bei GNN-Batches mit unterschiedlich langen Atom/Bond-Arrays fehlschlägt.
        out = model(x, training=False)
        if isinstance(out, (list, tuple)):
            import numpy as np
            out = np.concatenate([o.numpy() if hasattr(o, 'numpy') else o for o in out])
        else:
...
def predict_NMR_C(smiles, model):
    ...
    with open(os.path.join(_CASCADE_DIR, 'preprocessor.p'), 'rb') as ft:
