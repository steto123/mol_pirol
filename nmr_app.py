...
def init_models():
    ...
    try:
        ...
        # Neueste CSV-Datei im codes/-Ordner automatisch ermitteln
        _codes_dir = os.path.join(_NOTEBOOK_DIR, "codes")
        _csv_files = sorted(
            [f for f in Path(_codes_dir).glob("*.csv")],
            key=lambda f: f.stat().st_mtime,
            reverse=True
        ) if os.path.isdir(_codes_dir) else []
        if _csv_files:
            codefile = str(_csv_files[0])
            print(f"DCode file: {_csv_files[0].name}")
            codes_df = pd.read_csv(codefile, dtype={0: str})
        else:
            codes_df = None
            print(f"WARNING: No CSV file found in {_codes_dir}")
...
def predict_dcode_boltzmann(m, b_weights, codes_df_input, min_hashes=3):
    ...
    for cid, w in b_weights.items():
        ...
        for atom in single_conf_m.GetAtoms():
            if atom.GetAtomicNum() == 6 and atom.HasProp('DCode'):
                ...
                verschiebung, treffer, _, _, _ = calcshift(codes_df_input, codestring, atom.GetIdx(), min_hashes=min_hashes)
...
class CalculationWorker(QThread):
    ...
    def __init__(self, smiles, use_symmetry=False, min_hashes=3):
        super().__init__()
        self.smiles = smiles
        self.use_symmetry = use_symmetry
        self.min_hashes = min_hashes
    ...
    def run(self):
        try:
            ...
            pred_dcode = predict_dcode_boltzmann(m_3d, b_weights, codes_df, min_hashes=self.min_hashes) if codes_df is not None else {}
...
def load_config():
    """
    Loads config.txt from the application directory.
    Supported format:
    min_hashes = 3
    dark_mode = true / false
    symmetry_averaging = true / false
    """
    config = {
        'min_hashes': 3,
        'dark_mode': False,
        'symmetry_averaging': True
    }
    ...
    if os.path.exists(config_path):
            ...
            with open(config_path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if "=" in line:
                        ...
                        if key == "min_hashes":
                            try:
                                config['min_hashes'] = int(val)
                            except ValueError:
                                pass
                        elif key == "dark_mode":
                            config['dark_mode'] = val in ("true", "1", "yes", "on", "an")
                        elif key == "symmetry_averaging" or key == "symmetrie_averaging":
                            config['symmetry_averaging'] = val in ("true", "1", "yes", "on", "an")
        except Exception as e:
        ...
        try:
            with open(config_path, "w", encoding="utf-8") as f:
                f.write("# NMR Application Configuration File\n\n")
                f.write("# Minimale Anzahl an '#' im Suchstring (Standard = 3)\n")
                f.write("min_hashes = 3\n\n")
                f.write("# Dark Mode aktivieren (true/false)\n")
                f.write("dark_mode = false\n\n")
                f.write("# Symmetrie-Mittelwertbildung aktivieren (true/false)\n")
                f.write("symmetry_averaging = true\n")
        except Exception as e:
...
class NMRApp(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.dark_mode_cb.setChecked(self.config.get('dark_mode', False))
    ...
    def run_analysis(self):
        ...
        self.worker = CalculationWorker(
            smiles, 
            use_symmetry=self.config.get('symmetry_averaging', True), 
            min_hashes=self.config.get('min_hashes', 3)
        )
    ...
    def save_config(self):
        ...
        try:
            min_hashes = self.config.get('min_hashes', 3)
            dark_mode = "true" if self.dark_mode_cb.isChecked() else "false"
            sym_avg = "true" if self.config.get('symmetry_averaging', True) else "false"
            
            with open(config_path, "w", encoding="utf-8") as f:
                f.write("# NMR Application Configuration File\n\n")
                f.write("# Minimale Anzahl an '#' im Suchstring (Standard = 3)\n")
                f.write(f"min_hashes = {min_hashes}\n\n")
                f.write("# Dark Mode aktivieren (true/false)\n")
                f.write(f"dark_mode = {dark_mode}\n\n")
                f.write("# Symmetrie-Mittelwertbildung aktivieren (true/false)\n")
                f.write(f"symmetry_averaging = {sym_avg}\n")
        except Exception as e:
